param(
  [Parameter(Mandatory = $false)][string]$HookUrl = $env:VERCEL_DEPLOY_HOOK_DEVELOP
)

if (-not $HookUrl -or $HookUrl.Trim().Length -eq 0) {
  Write-Error "Provide the Vercel Deploy Hook URL via -HookUrl or VERCEL_DEPLOY_HOOK_DEVELOP env var."
  exit 1
}
