...
// pagos pendientes con fecha posterior a su fecha_renovacion.
// Dispara el trigger fn_generar_pagos_programados haciendo un no-op UPDATE en dia_pago.
require('dotenv').config({ path: '.env.local' });
const { Client } = require('pg');

async function main() {
  const url = process.env.DevDATABASE_URL;
  if (!url) {
    console.error('❌ DevDATABASE_URL no está definida en .env.local');
    process.exit(1);
  }
  ...
  try {
    ...
    // 1. Primero: re-aplicar la migración (asegura que la función es la versión fija)
    const fs = require('fs');
    ...
    // 3. Por cada póliza: hacer un no-op UPDATE en dia_pago para disparar el trigger
    let fixed = 0;
