...
export default function ProspectosPage(){
  ...
  {agg && (!superuser || (superuser && agenteId)) && <div className="d-flex flex-column gap-2 small mb-3">
        ...
        {(!superuser || (superuser && agenteId)) && (()=>{ const carry = activosPrevios.length; const objetivo = metaProspectos + carry; const progreso = actuales.length; const pct = Math.min(100,(progreso/objetivo)*100); const ok = progreso>=objetivo; return <div style={{minWidth:300}} className="d-flex align-items-center gap-3">
          <div className="flex-grow-1">
            <div className="progress" role="progressbar" aria-valuenow={progreso} aria-valuemin={0} aria-valuemax={objetivo} title={`Progreso semanal ${progreso}/${objetivo}`}>
              <div className={`progress-bar ${ok? 'bg-success':'bg-warning text-dark'}`} style={{width: pct+'%'}} />
            </div>
          </div>
          <div style={{minWidth:70}} className="text-nowrap fw-semibold small">{progreso}/{objetivo}</div>
        </div>})()}
    </div>}
