...
export default function ProspectosPage() {
  ...
  if(agrupado){
    ...
    // Resumen local solo del agente (evita usar aggregate global por error de filtro)
    const resumenLocal = (()=>{
      const counts: Record<string,number> = { pendiente:0, seguimiento:0, con_cita:0, descartado:0 }
      for(const p of prospectos){ if(counts[p.estado]!==undefined) counts[p.estado]++ }
      return { total: prospectos.length, por_estado: counts, cumplimiento_30: prospectos.length>=30 }
    })()
    const extended = computeExtendedMetrics(prospectos,{ diaSemanaActual })
  exportProspectosPDF(prospectos, resumenLocal, titulo, { incluirId:false, agrupadoPorAgente: agrupado, agentesMap, chartEstados: true, metaProspectos, metaCitas, forceLogoBlanco:true, extendedMetrics: extended, prevWeekDelta: agg && prevAgg? computePreviousWeekDelta(agg, prevAgg): undefined, filename })
  }
  }}>PDF</button>
    </div>}
