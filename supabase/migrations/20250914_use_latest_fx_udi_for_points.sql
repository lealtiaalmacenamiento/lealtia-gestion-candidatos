...
BEGIN
  ...
  -- Tasas aplicadas para auditoría (usando valores actuales)
  ...
  -- Auto-selección de variante si no viene asignada (usar SA MXN con valor actual cuando aplique)
  ...
  ELSE
    ...
    -- Clasificación por rangos en MXN (con prima_mxn actualizada)
  ...
  -- Año de vigencia (sigue basado en fecha_emision)
...
-- Backfill: recomputa todas las pólizas para reflejar FX/UDI actual en cache
DO $$
DECLARE
  v int;
BEGIN
