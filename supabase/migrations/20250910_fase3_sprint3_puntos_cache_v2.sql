-- Fase 3 – Sprint 3: Puntos y caché (complemento)
-- Fecha: 2025-09-10
-- Objetivo: completar piezas faltantes
--  - Guardar tasas aplicadas (UDI/FX) en cache
--  - Disparar recálculo también al cambiar SA
--  - Selección automática de variante de producto si producto_parametro_id es NULL
--  - Función de recálculo masivo

-- 1) Actualización de la función recalc_puntos_poliza: llenar udi_valor/usd_fx y auto-selección de producto
...
BEGIN
  ...
  -- luego mayor sa_min (más específica). Si hay empate o no se encuentra, se deja NULL.
  IF v_pp_id IS NULL THEN
    ...
    IF v_pp_auto IS NOT NULL THEN
      v_pp_id := v_pp_auto;
      -- Persistir selección para futuras evaluaciones
...
-- 2) Disparar recálculo también al cambiar SA y fecha_emision (impacta year y normalización)
